...
Annotate any method with the `#[Test]` attribute and it will be picked up as a test. Then use the `test()` function to make assertions.
...
To use another container instead of Tempest's, you can create a new initializer class anywhere in your project that returns an instance of `Tempest\Testing\Actions\RunTest`.
...
The built-in traits to test Tempest-specific code are still work in progress.
